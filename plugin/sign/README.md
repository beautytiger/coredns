 ...
 *  (Re)-sign the zone with the CSK(s) when:
     ...
     -  the last time it was signed is more than a 6 days ago. Each zone will have some jitter (0, 3
        days) applied.

     -  the signature only has 14 days left before expiring.

    Both these dates are checked on the SOA's signature(s).

 *  Create signatures that have an inception of -3 hours and a expiration of +32 days for every key
    given.
