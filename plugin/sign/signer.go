...
func signAndLog(s *Signer, why error) {
	...
	log.Infof("Successfully signed zone %q in %q with key tags %q and %d SOA serial, elapsed %s, next: %s", s.origin, filepath.Join(s.directory, s.signedfile), keyTag(s.keys), z.Apex.SOA.Serial, time.Since(now), now.Add(DurationRefreshHours).Format(timeFmt))
